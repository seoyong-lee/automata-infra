...
- Shotstack MVP는 `render-plan -> Shotstack edit JSON` mapper를 통해서만 붙이고, source asset은 CloudFront preview domain 기반 URL로 해석한다
- Shotstack 결과물은 provider temporary URL에 의존하지 않고 다시 S3 `rendered/` 및 `previews/` 경로로 복사해 후속 review/upload 경로를 안정화한다
