import {
  resolveElevenLabsCharAlignment,
  type ElevenLabsCharAlignment,
  type ElevenLabsStoredAlignmentDocument,
...
/** Loose compare: ignore common punctuation / symbols that often differ between UI copy and vendor alignment. */
const alignmentTextKey = (s: string): string =>
  canonText(s).replace(/[\s.,!?…:;，。、？！…"'""''()[\]{}\-_/\\]+/g, "");
...
const pickPhraseSourceForAlignment = (input: {
  ...
  const keyStrict = canonText(input.rawAligned);
  const keyLoose = alignmentTextKey(input.rawAligned);
  ...
  /** Persisted `sourceText` is usually the exact TTS request string. */
  push(input.sourceText);
  push(input.narration);
  push(input.subtitle);
  for (const candidate of tryKeys) {
    if (canonText(candidate) === keyStrict) {
      return candidate;
    }
    if (alignmentTextKey(candidate) === keyLoose && keyLoose.length > 0) {
      return candidate;
    }
  }
  return undefined;
...
const mapPhrasePartsToGlobalSegments = (input: {
  parts: string[];
  rawAligned: string;
  alignment: ElevenLabsCharAlignment;
  ...
  const { parts, rawAligned, alignment, sceneStartSec, sceneEndSec } = input;
...
const resolveOverlappingSegmentEnds = (
  segments: RenderPlanSubtitleSegment[],
  sceneEndSec: number,
): void => {
...
export const tryBuildSubtitleSegmentsFromElevenLabs = (input: {
  ...
  const segments = mapPhrasePartsToGlobalSegments({
    parts,
    rawAligned,
    alignment,
    sceneStartSec,
  ...
  resolveOverlappingSegmentEnds(segments, sceneEndSec);
  return segments;
};
